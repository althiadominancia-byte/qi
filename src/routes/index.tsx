...
import {
  ...
  Loader2, Briefcase, AlertCircle, Lightbulb, ThumbsUp,
} from "lucide-react";
...
import {
  ...
  DISC_BLOCKS, SITUACIONAIS, PERFIS, DIM_INFO,
  COR_RACA, GENERO, ORIENTACAO, PCD, POLITICO,
  computeResults, corNivel,
} from "@/lib/recrutamento/data";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Vaga Televendas — Distribuidora Estrela" },
      { name: "description", content: "Inscreva-se no processo seletivo interno de Televendas da Distribuidora Estrela." },
    ],
  }),
...
function Card({ children }: any) {
  return <div style={{ background: "#fff", borderRadius: 18, padding: 24, border: `1px solid ${BORDA}`, boxShadow: "0 8px 30px -12px rgba(80,50,138,.18)", marginBottom: 14 }}>{children}</div>;
}
function Badge({ children }: any) {
  return <span style={{ fontSize: 11.5, fontWeight: 700, background: LARANJA_TINT, color: LARANJA, padding: "5px 11px", borderRadius: 99, letterSpacing: 0.5 }}>{children}</span>;
}
...
function Botao({ children, onClick, disabled }: any) {
  return <button type="button" disabled={disabled} onClick={onClick} style={{
    background: disabled ? "#D8D2E6" : LARANJA, color: "#fff", border: "none", padding: "13px 22px", borderRadius: 12,
    fontSize: 15, fontWeight: 700, cursor: disabled ? "wait" : "pointer", display: "inline-flex", alignItems: "center", gap: 7, fontFamily: "inherit",
...
const FLOW = ["intro", "dados", "curriculo", "situacional", "disc", "diversidade", "revisao", "resultado"];
const FORM_STEPS = ["dados", "curriculo", "situacional", "disc", "diversidade", "revisao"];
...
function FormularioPage() {
  ...
  const resultado = useMemo(() => (step === "resultado" ? computeResults(a) : null), [step, a]);
  ...
  async function enviarInscricao() {
    ...
    try {
      ...
      // 1. Upload do CV (se houver) para storage privado
      ...
      // 2. Insere diversidade SEPARADAMENTE (sem vínculo)
      ...
      // 4. Vai para tela de resultado e dispara análise IA em background
      setStep("resultado");
      window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  async function rodarAnalise(candId: string, cvPath: string | null, cvMime: string | null) {
    ...
    try {
      ...
      const parsed = await analisarCv({
        data: {
          storagePath: textoBruto ? null : cvPath,
          mimeType: textoBruto ? null : cvMime,
          textoExtra: textoExtra(),
          textoBruto,
        },
      });
      setCvAnalysis(parsed);
      // Persiste no banco (best-effort, anon não tem update — então só apresentação local)
  ...
  useEffect(() => {
    ...
    if (cvAnalysis || cvLoading || cvError) return;
    // CV analysis é disparada por enviarInscricao(); este efeito só evita re-disparos
  }, [step, cvAnalysis, cvLoading, cvError]);

  const reset = () => {
    setA({}); setStep("intro");
    setCvFile(null); setCvAnalysis(null); setCvError(""); setCvLoading(false); setSubmitError("");
  };
    ...
    <div style={{
      ...
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@500;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
        *{box-sizing:border-box}
        input:focus,select:focus,textarea:focus{border-color:${ROXO}!important;box-shadow:0 0 0 3px ${ROXO_TINT2}}
      ...
      <div style={{ background: ROXO, padding: "16px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ marginLeft: "auto", color: "#fff", fontSize: 12, opacity: 0.8, display: "flex", alignItems: "center", gap: 6 }}>
          <Headphones size={15} /> Processo Interno · Televendas
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 18px" }}>
        ...
        {step === "intro" && (
          <Card>
            <Badge>Vaga Interna</Badge>
            <h1 className="h" style={{ fontSize: 27, fontWeight: 800, margin: "14px 0 8px", lineHeight: 1.15 }}>
              Quer fazer parte do nosso time de <span style={{ color: LARANJA }}>Televendas</span>?
            </h1>
            <p style={{ color: CINZA, fontSize: 15, lineHeight: 1.6, margin: 0 }}>
              Este formulário abre o processo seletivo interno da Distribuidora Estrela. Vamos conhecer seu currículo
              e entender o seu jeito de atender. Leva cerca de <strong>8 minutos</strong>. Responda com sinceridade —
            ...
            <Botao onClick={next}>Começar <ChevronRight size={18} /></Botao>
          </Card>
        )}
        ...
        {step === "dados" && (
          <Card>
            ...
            <Campo icon={User} label="Nome completo" obrig>
              <input style={inputStyle} value={a.nome || ""} onChange={(e) => set("nome", e.target.value)} placeholder="Seu nome" />
            </Campo>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Campo icon={Mail} label="E-mail" obrig>
                <input style={inputStyle} type="email" value={a.email || ""} onChange={(e) => set("email", e.target.value)} placeholder="voce@email.com" />
              </Campo>
              <Campo icon={Phone} label="Celular / WhatsApp" obrig>
                <input style={inputStyle} value={a.celular || ""} onChange={(e) => set("celular", e.target.value)} placeholder="(96) 9 9999-9999" />
              </Campo>
            </div>
            <Campo icon={MapPin} label="Endereço (bairro e cidade)">
              <input style={inputStyle} value={a.endereco || ""} onChange={(e) => set("endereco", e.target.value)} placeholder="Bairro, Cidade - UF" />
            </Campo>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Campo label="Setor / função atual">
                <input style={inputStyle} value={a.setor || ""} onChange={(e) => set("setor", e.target.value)} placeholder="Ex.: Estoque, Caixa..." />
              </Campo>
              <Campo label="Tempo de empresa">
                <select style={inputStyle} value={a.tempo || ""} onChange={(e) => set("tempo", e.target.value)}>
                  <option value="">Selecione</option>
                  <option>Menos de 6 meses</option><option>6 meses a 1 ano</option>
                  <option>1 a 3 anos</option><option>Mais de 3 anos</option>
                </select>
              </Campo>
            </div>
        ...
        {step === "curriculo" && (
          <Card>
            ...
            <Campo icon={Upload} label="Anexar currículo (PDF, Word ou imagem)">
              <div style={{ border: `2px dashed ${cvFile ? ROXO : BORDA}`, borderRadius: 13, padding: 22, textAlign: "center", background: ROXO_TINT }}>
                ...
                <div style={{ fontSize: 13.5, fontWeight: 600, color: ROXO_DARK }}>
                  {a.cvNome ? `📎 ${a.cvNome}` : "Clique para selecionar o arquivo"}
                </div>
            ...
            <Campo label="Já trabalhou com vendas, atendimento ou telefone? Conte rapidamente.">
              <textarea style={{ ...inputStyle, minHeight: 88, resize: "vertical" }} value={a.exp || ""} onChange={(e) => set("exp", e.target.value)} placeholder="Ex.: 1 ano no caixa atendendo clientes, vendas no balcão..." />
            </Campo>
            <Campo label="Por que você quer atuar em televendas?">
            ...
            {SITUACIONAIS.map((q, i) => (
              <div key={q.id} style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {q.options.map((o) => (
                    <Pill key={o.key} ativo={a["sit_" + q.id] === o.key} onClick={() => set("sit_" + q.id, o.key)}>{o.txt}</Pill>
                  ))}
                </div>
              </div>
            ))}
        ...
        {step === "disc" && (
          <Card>
            <Titulo icon={Brain} sub="Em cada bloco, marque a frase que MAIS combina e a que MENOS combina com você. Todas são qualidades — vá pelo seu jeito, não pela resposta que parece “ideal”.">Seu estilo</Titulo>
            {DISC_BLOCKS.map((b, bi) => (
              <div key={bi} style={{ marginBottom: 16, padding: 14, borderRadius: 14, background: ROXO_TINT }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, fontWeight: 700, color: ROXO, marginBottom: 10 }}>
                  <span>BLOCO {bi + 1} DE {DISC_BLOCKS.length}</span>
          ...
          <Card>
            ...
            <div style={{ background: LARANJA_TINT, border: `1.5px solid ${LARANJA}33`, borderRadius: 12, padding: 14, display: "flex", gap: 11, marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12.5, color: ROXO_DARK, lineHeight: 1.55 }}>
                Esta etapa é <strong>100% opcional e sigilosa</strong>. As respostas <strong>não influenciam em nada</strong> a sua
                avaliação no processo — servem apenas para a empresa acompanhar a diversidade do time, conforme a LGPD.
                Você pode marcar <em>“Prefiro não responder”</em> em qualquer pergunta.
              </div>
            </div>
        ...
        {step === "revisao" && (
          <Card>
            ...
            <Linha k="Situações respondidas" v={`${SITUACIONAIS.filter((q) => a["sit_" + q.id]).length}/${SITUACIONAIS.length}`} />
        ...
        {step === "resultado" && resultado && (
          <>
            <Card>
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ color: CINZA, fontSize: 14, margin: 0, lineHeight: 1.55 }}>
                  Recebemos sua candidatura para o time de Televendas. O RH vai analisar e entrar em contato pelo
                  e-mail e WhatsApp informados. Boa sorte!
                </p>
              </div>
            </Card>
            ...
            <div style={{ margin: "10px 0 14px", display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700, color: ROXO, textTransform: "uppercase", letterSpacing: 1 }}>
              <BarChart3 size={15} /> Pré-visualização da análise · visão do recrutador
            </div>
            ...
            <Card>
              ...
              {cvLoading && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, color: ROXO, fontSize: 14, fontWeight: 600, padding: "10px 0" }}>
                  <Loader2 size={20} className="spin" /> Lendo e analisando o currículo...
                </div>
              )}
              {cvError && !cvLoading && (
                ...
                <div>
                  ...
                  {(cvAnalysis.perguntas_entrevista || []).length > 0 && (
                    <div style={{ marginTop: 14, background: ROXO_TINT, borderRadius: 12, padding: 14 }}>
                      <div className="h" style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 700, fontSize: 13, marginBottom: 9, color: ROXO }}><Lightbulb size={15} /> Sugestões para a entrevista</div>
                      {cvAnalysis.perguntas_entrevista.map((p: string, i: number) => (
                        <div key={i} style={{ fontSize: 12.5, color: CINZA, marginBottom: 6, display: "flex", gap: 6 }}><span style={{ color: ROXO }}>{i + 1}.</span> {p}</div>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <button type="button" onClick={reset} style={{ margin: "16px auto 0", display: "block", background: "none", border: "none", color: ROXO, fontWeight: 600, fontSize: 13, cursor: "pointer", textDecoration: "underline" }}>↺ Refazer / nova inscrição</button>
          </>
        )}
      </div>
    </div>
