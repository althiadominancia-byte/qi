...
const DESTAQUES = [
  {
    icon: Sparkles,
    titulo: "Análise de currículo com IA",
    texto: "O CV é lido por IA e recebe uma nota de match contra a vaga.",
  },
  {
    icon: Target,
    titulo: "Match automático da vaga",
    texto: "DISC, questões situacionais e perfil combinados em um só score.",
  },
  {
    icon: ShieldCheck,
    titulo: "Multiempresa e LGPD",
    texto: "Isolamento por empresa e tratamento de dados em conformidade.",
  },
];
...
function AuthPage() {
  ...
  // Roteia pela identidade: staff (usuarios) → /super|/admin; candidato → /portal.
  async function rotearPorIdentidade() {
    const destino = await destinoPorIdentidade();
    // Auth-user órfão fica em /auth (evita loop de navegação).
    if (destino === "/auth") return;
  ...
  useEffect(() => {
    const {
      ...
      if (session) rotearPorIdentidade();
    });
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) rotearPorIdentidade();
    });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (/invalid login credentials/i.test(raw)) text = "E-mail ou senha incorretos.";
        ...
        text = "E-mail ainda não confirmado. Verifique sua caixa de entrada.";
  ...
  async function esqueciSenha() {
    if (!email) {
      setMsg({ type: "err", text: "Informe seu e-mail acima para receber o link." });
      return;
    }
    setLoading(true);
    setMsg(null);
      ...
      >
        ...
        <div
          style={{
            ...
            height: 340,
            borderRadius: "50%",
            background: `radial-gradient(circle, ${LARANJA}55 0%, transparent 68%)`,
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 13 }}>
          ...
          <div style={{ lineHeight: 1.15 }}>
            <div className="h" style={{ fontWeight: 800, fontSize: 18, letterSpacing: 0.3 }}>
              Recrutamento
            </div>
            <div
              style={{
        ...
        <div style={{ position: "relative", margin: "auto 0" }}>
          ...
          >
            Gerencie vagas, candidatos e a estrutura da sua empresa com avaliação inteligente de
            perfil e match automático.
          </p>
          ...
          <div style={{ display: "grid", gap: 16, maxWidth: 440 }}>
            {DESTAQUES.map(({ icon: Ic, titulo, texto }) => (
              <div key={titulo} style={{ display: "flex", gap: 13, alignItems: "flex-start" }}>
                ...
                <div>
                  <div className="h" style={{ fontWeight: 700, fontSize: 14.5 }}>
                    {titulo}
                  </div>
      ...
      >
        <form onSubmit={submit} style={{ width: "100%", maxWidth: 380 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 30 }}>
            ...
            <div className="h" style={{ fontWeight: 800, fontSize: 17 }}>
              Recrutamento
            </div>
          </div>

          <h2 className="h" style={{ fontSize: 25, fontWeight: 800, margin: "0 0 6px" }}>
            Entrar no painel
          </h2>
          ...
          {/* Acesso do candidato ao portal (rota fora do routeTree tipado — âncora simples). */}
          <div style={{ marginTop: 14, textAlign: "center" }}>
            ...
            >
              É candidato? Acompanhe sua candidatura
            </a>
          </div>
          ...
          >
            ...
            <strong style={{ color: CINZA }}>LGPD</strong>.
          </div>
        </form>
      </div>
    </div>
