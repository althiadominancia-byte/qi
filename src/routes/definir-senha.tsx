...
export const Route = createFileRoute("/definir-senha")({
  ssr: false,
  head: () => ({ meta: [{ title: "Definir senha · Plataforma de Recrutamento" }] }),
...
function DefinirSenhaPage() {
    ...
    (async () => {
      try {
        ...
        if (!u.user) {
          setMsg({
            type: "err",
            text: "Link inválido ou expirado. Peça um novo convite ao administrador.",
          });
          setLoading(false);
          return;
        }
  ...
  async function submit(e: React.FormEvent) {
    ...
    setMsg(null);
    if (senha.length < 8) {
      setMsg({ type: "err", text: "A senha deve ter pelo menos 8 caracteres." });
      return;
    }
    if (senha !== senha2) {
    ...
    try {
      ...
      // Staff vence (convite → /admin|/super); candidato de "esqueci senha" → /portal.
      const destino = await destinoPorIdentidade();
      // "/portal" ainda não está no routeTree gerado — cast até a rota existir.
      setTimeout(() => navigate({ to: destino as any, replace: true }), 800);
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Marca neutra da plataforma (sem marca de tenant) — mesma linguagem de auth.tsx. */}
        <div
          style={{
            width: 32,
            height: 32,
        ...
        <div style={{ lineHeight: 1 }}>
          ...
          <div className="h" style={{ color: "#fff", fontWeight: 800, fontSize: 17 }}>
            Definir sua senha
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          <h1 className="h" style={{ fontSize: 22, fontWeight: 800, margin: "0 0 6px" }}>
            {tipo === "recovery" ? "Redefinir senha" : "Bem-vindo(a) à plataforma"}
          </h1>
          <p style={{ color: CINZA, fontSize: 13, margin: "0 0 20px" }}>
            {tipo === "recovery"
              ? "Crie uma nova senha para acessar sua conta."
              : "Este é seu primeiro acesso. Crie uma senha para começar."}
          </p>
