...
export const FEATURE_LABELS: Record<FeatureKey, { nome: string; desc: string }> = {
  ...
  avaliacao_experiencia: { nome: "Avaliação de experiência", desc: "Acompanhamento 30/60/90 pós-contratação" },
  ...
  entrevista_ia:         { nome: "Entrevista por vídeo com IA", desc: "Sala de vídeo + análise de conteúdo" },
};

// Modelos padrão — servem para semear a tabela `planos` e como referência.
export type PlanoModelo = { nome: string; descricao: string; features: Record<FeatureKey, boolean> };
...
export const PLAN_PRESETS: Record<"basico" | "pro" | "enterprise", PlanoModelo> = {
  basico: {
    ...
    features: { ...allFalse(), disc: true, situacional: true },
  },
  pro: {
    nome: "Pro",
    ...
    features: {
      ...allFalse(),
      analise_cv_ia: true, disc: true, situacional: true, diversidade: true,
      avaliacao_experiencia: true, niveis_lideranca: true, multiplas_unidades: true, exportacao: true,
    },
  },
  enterprise: {
    nome: "Enterprise",
...
export function resolveFeatures(
  ...
  for (const k of FEATURE_KEYS) {
    if (semPlano && semOverride) { out[k] = true; continue; }
