...
BEGIN
  ...
  SELECT v.empresa_id, v.unidade_id
    INTO v_emp, v_un
    FROM public.vagas v
   WHERE v.id = NEW.vaga_id
     AND public.vaga_aceita_inscricao(v.id);
...
DROP TRIGGER IF EXISTS trg_fill_candidato ON public.candidatos_televendas;
...
FOR EACH ROW
EXECUTE FUNCTION public.fill_candidato_televendas_from_vaga();
...
FOR INSERT
TO anon
WITH CHECK (vaga_id IS NOT NULL AND public.vaga_aceita_inscricao(vaga_id));
